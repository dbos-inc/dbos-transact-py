...
class DBOS:
    ...
    def set_workflow_delay(
        ...
        *,
        ...
        def fn() -> None:
            _get_dbos_instance()._sys_db.set_workflow_delay(
                workflow_id,
    ...
    async def patch_async(cls, patch_name: str) -> bool:
        ...
        # consume a checkpoint position, because sibling tasks in the same
        # async workflow may reserve positions while this coroutine is off
        # the event loop. Instead, probe the database in a thread, then
        # revalidate and reserve the position on the event loop, where the
        # reservation is atomic with respect to all sibling tasks.
        while True:
            function_id = ctx.function_id
            ...
            # No awaits may occur between this revalidation and the
            # reservation below, so the slice is atomic on the event loop.
            if ctx.function_id != function_id:
                # A sibling task consumed positions during the probe; retry.
                continue
