...
def test_blocked_event_loop_does_not_stall_dispatch(dbos: DBOS) -> None:
    """Handing an async workflow to the event loop must not block the queue worker.

    The loop runs every async workflow in the process, so waiting on it to accept a
    handoff makes one loop-blocking workflow stall dispatch of everything behind it,
    including sync workflows that never touch the loop.
    """
    loop_blocked = threading.Event()
    release = threading.Event()
    tail_ran = threading.Event()

    @DBOS.workflow()
    async def loop_blocker() -> None:
        loop_blocked.set()
        # Deliberately synchronous: occupies the event loop thread itself.
        release.wait(timeout=30)

    @DBOS.workflow()
    async def async_follower() -> None:
        return
    ...
    queue = Queue(f"loopblock_{uuid.uuid4().hex[:8]}", polling_interval_sec=0.25)
    blocker_handle = queue.enqueue(loop_blocker)
    # Only enqueue the rest once the loop is definitely blocked, so the dispatch of
    # async_follower is what the queue worker would otherwise wait on.
    assert loop_blocked.wait(timeout=15)
    queue.enqueue(async_follower)
    tail_handle = queue.enqueue(sync_tail)

    try:
        assert tail_ran.wait(
            timeout=15
        ), "queue worker stalled behind a blocked event loop"
    finally:
        release.set()
    assert tail_handle.get_result() is None
    assert blocker_handle.get_result() is None
    assert queue_entries_are_cleaned_up(dbos)
