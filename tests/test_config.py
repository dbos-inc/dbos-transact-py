...
def test_no_config_provided(mocker):
    ...
    assert (
        dbos.config["database_url"]
        == f"postgresql://postgres:{os.environ.get('PGPASSWORD', 'dbos')}@localhost:5432/some_db?connect_timeout=3"
    )
...
def test_configfile_type_provided():
    ...
    assert (
        dbos.config["database_url"]
        == f"postgresql://postgres:{os.environ.get('PGPASSWORD', 'dbos')}@localhost:5432/some_app?connect_timeout=3"
    )
    assert dbos.config["database"]["app_db_pool_size"] == 20
    assert dbos.config["database"]["sys_db_pool_size"] == 20
...
def test_dbosconfig_type_provided():
    config: DBOSConfig = {
        "name": "some-app",
        "database_url": f"postgresql://postgres:{os.environ.get('PGPASSWORD', 'dbos')}@localhost:5432/some_app",
    }
    dbos = DBOS(config=config)
    assert dbos.config["name"] == "some-app"
    assert dbos.config["database_url"] == config["database_url"]
...
def test_overwrite_config(mocker):
    ...
    assert (
        config["database_url"]
        == "postgresql://dbosadmin:pwd@hostname:1234/appdbname?connect_timeout=3&sslmode=no-verify"
    )
...
def test_overwrite_config_with_ca(mocker):
    ...
    assert (
        config["database_url"]
        == "postgresql://dbosadmin:pwd@hostname:1234/appdbname?connect_timeout=3&sslmode=verify-full&sslrootcert=cert.pem"
    )
